...
def projectioncall(projectioninput):
    """ you can only name your projection 'hammer', 'aitoff', 'mollweide', 'lambert'
    """
    assert projectioninput=='hammer'
    assert projectioninput=='aitoff'
    assert projectioninput=='mollweide'
    assert projectioninput=='lambert'
    ...
    try:
        ...
        ax = plt.axes(projection=projectioninput)
        ...
        plt.savefig(projectioninput)#"Problem1_SDSS.png")
...
projectioncall('hammer')
projectioncall('aitoff')
projectioncall('mollweide')
projectioncall('lambert')
projectioncall('sillyprojection')